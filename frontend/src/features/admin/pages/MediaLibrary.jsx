...
export default function MediaLibrary() {
  ...
  const createFolder = async () => {
    const name = window.prompt(t("media.folderName"));
    if (!name) return;
    try {
    ...
    <div data-testid={MEDIA.page}>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={createFolder} data-testid={MEDIA.newFolderButton} className="kti-focus inline-flex items-center gap-2 rounded-full border border-white/12 px-4 py-2.5 text-sm text-white hover:bg-white/[0.06]">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button onClick={doDelete} data-testid={MEDIA.deleteButton} className="kti-focus inline-flex items-center gap-1.5 rounded-lg border border-[rgba(255,92,122,0.4)] px-3 py-2 text-xs text-[#ff96aa] hover:bg-[rgba(255,92,122,0.1)]"><Trash2 className="size-3.5" /> {t("media.delete")}</button>
            </div>
          </div>
