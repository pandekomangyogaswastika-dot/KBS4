...
export default function AdminProjects() {
  ...
  const deleteMilestone = async (pid, mid) => {
    if (!window.confirm("Hapus milestone?")) return;
  ...
  const decideApproval = async (pid, aid, status) => {
    const feedback = status === "changes_requested" ? window.prompt("Berikan feedback:") : "";
    try {
      await api.patch(`/projects/${pid}/approvals/${aid}`, { status, feedback: feedback || "" });
              ...
              <div key={p.id} className="rounded-2xl border border-white/8 bg-white/[0.04] overflow-hidden">
                ...
                <div className="flex items-center gap-4 px-5 py-4">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {user?.role === "admin" && <button onClick={() => deleteProject(p.id)} className="grid size-8 place-items-center rounded-lg text-[color:var(--kti-text-dim)] hover:bg-red-500/20 hover:text-red-400"><Trash2 className="size-4" /></button>}
                          ...
                          {msList.map((m) => {
                            ...
                            return (
                              <div key={m.id} className="flex items-center gap-3 rounded-xl border border-white/8 bg-white/[0.03] px-4 py-3">
                                ...
                                <button onClick={() => deleteMilestone(p.id, m.id)} className="grid size-7 place-items-center rounded-lg text-[color:var(--kti-text-dim)] hover:text-red-400"><Trash className="size-3.5" /></button>
                              </div>
                            );
                          })}
                          {addingMs[p.id] ? (
                        ...
                        <div className="space-y-3">
                          {docList.map((d) => (
                            <div key={d.id} className="flex items-center gap-3 rounded-xl border border-white/8 bg-white/[0.03] px-4 py-3">
                              ...
                              <button onClick={() => deleteDoc(p.id, d.id)} className="grid size-7 place-items-center rounded-lg text-[color:var(--kti-text-dim)] hover:text-red-400"><Trash className="size-3.5" /></button>
                            </div>
                          ))}
