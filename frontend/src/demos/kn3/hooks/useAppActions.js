...
export function useAppActions(state) {
  ...
  const submitOrder = async () => {
    try {
      const payload = {
        ...
        items: cart.map((item) => ({ product_id: item.product.id, quantity: item.quantity, unit: item.unit })),
      };
      const response = await axios.post(`${API}/sales-orders`, payload);
      setNotice(`${response.data.number} dibuat. Status: ${response.data.status.replaceAll("_", " ")}.`);
      setCart([]);
  ...
  const payInvoice = async (orderId, amount) => {
    try {
      const response = await axios.post(`${API}/sales-orders/${orderId}/simulate-payment`, { amount, method: "Transfer Simulasi", created_by: user?.name || "Admin Demo" });
      setNotice(`Invoice ${response.data.number} dibuat. MOCKED/SIMULATED payment.`);
      await loadAll();
    } catch (error) {
