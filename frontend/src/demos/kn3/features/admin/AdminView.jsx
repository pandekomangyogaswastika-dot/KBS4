...
export default function AdminView({
  ...
  const [product, setProduct] = useState({ sku: "", name: "", category: "Batik", variant: "Regular", color: "", motif: "", grade: "A", supplier: "", base_unit: "meter", price: 0 });
  const [customer, setCustomer] = useState({ name: "", pic_name: "", phone: "", city: "Jakarta", address: "" });
  ...
  const tabs = [
    ["products", "Product"], ["customers", "Customer"], ["warehouses", "Warehouse"], ["uoms", "UOM"], ["templates", "Templates"], ["permissions", "Permissions"], ["audit", "Audit"], ["users", "Users"],
  ];
    ...
    <div data-testid="admin-view">
      ...
      <section className="grid gap-3 lg:grid-cols-[360px_1fr]">
        ...
        <div className="section-card">
          ...
          {!["users", "permissions", "audit"].includes(tab) && <div data-testid="admin-import-export-panel" className="mb-3 grid gap-2 rounded-md border border-[#EFF0F2] bg-[#FAFBFC] p-2.5">
            ...
            {importPreview && (
              <div data-testid="import-preview-result" className="rounded-md border border-[#EFF0F2] bg-white p-2 text-[11.5px]">
                ...
                <p className="text-green-700">✓ Akan dibuat: {importPreview.created}</p>
                <p className="text-blue-700">~ Akan diupdate: {importPreview.updated}</p>
                {(importPreview.errors || []).length > 0 && (
                  <div className="mt-1 max-h-24 overflow-auto">
                    <p className="text-red-700 font-bold">⚠ {importPreview.errors.length} error:</p>
          ...
          {tab === "products" && <div className="grid gap-2">
            {[["sku", "SKU"], ["name", "Nama produk"], ["category", "Kategori"], ["variant", "Varian"], ["color", "Warna"], ["motif", "Motif"], ["grade", "Grade"], ["supplier", "Supplier"], ["base_unit", "Base unit"], ["price", "Harga"]].map(([key, ph]) => <input key={key} data-testid={`admin-product-${key}-input`} className="field" placeholder={ph} value={product[key]} onChange={(e) => setProduct({ ...product, [key]: key === "price" ? Number(e.target.value) : e.target.value })} />)}
            <button data-testid="admin-create-product-button" className="primary-button" onClick={() => onAdminCreate("products", product)}><Save size={14} /> Simpan Product</button>
          </div>}
          {tab === "customers" && <div className="grid gap-2">
            {[["name", "Nama customer"], ["pic_name", "PIC"], ["phone", "Phone"], ["city", "Kota"], ["address", "Alamat"]].map(([key, ph]) => <input key={key} data-testid={`admin-customer-${key}-input`} className="field" placeholder={ph} value={customer[key]} onChange={(e) => setCustomer({ ...customer, [key]: e.target.value })} />)}
          ...
          {tab === "templates" && <div className="grid gap-2">
            ...
            <div data-testid="template-section-order-editor" className="rounded-md border border-[#EFF0F2] bg-[#FAFBFC] p-2.5">
              ...
              {template.section_order.map((section) => <div key={section} data-testid={`template-section-${section}`} draggable className="mb-1.5 flex items-center justify-between rounded-md bg-white px-2 py-1 text-[12px] font-semibold border border-[#EFF0F2]"><span>{section}</span><span className="flex gap-1"><button data-testid={`template-section-${section}-up-button`} className="secondary-button" onClick={() => moveSection(section, -1)}>↑</button><button data-testid={`template-section-${section}-down-button`} className="secondary-button" onClick={() => moveSection(section, 1)}>↓</button></span></div>)}
            </div>
        ...
        <div className="section-card">
          ...
          <div className="grid gap-2">
            {(tab === "products" ? data.products : tab === "customers" ? data.customers : tab === "warehouses" ? data.warehouses : tab === "uoms" ? uoms : tab === "templates" ? templates : users).map((row) => (
              <div data-testid={`admin-record-${tab}-${row.id}`} key={row.id} role="button" tabIndex={0} className="rounded-md border border-[#EFF0F2] bg-[#FAFBFC] interactive-card flex flex-col gap-2 p-2.5 md:flex-row md:items-center md:justify-between" onClick={() => onShowDetail({ title: row.name || row.code || row.email, body: `Record ${tab} ini dapat diupdate, dinonaktifkan, atau diexport dari Admin CRUD.`, facts: [{ label: "Module", value: tab }, { label: "Status", value: row.status || (row.active === false ? "inactive" : "active") }], target: "admin", cta: "Tetap di Admin" })}>
                <div className="min-w-0">
                  <p data-testid={`admin-record-title-${row.id}`} className="text-[12.5px] font-semibold truncate">{row.name || row.code || row.email}</p>
                  <p data-testid={`admin-record-meta-${row.id}`} className="text-[11px] text-[#3C3C43] truncate">{row.sku || row.code || row.document_type || row.role || row.city} • {row.status || (row.active === false ? "inactive" : "active")}</p>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  <button data-testid={`admin-update-${tab}-${row.id}-button`} className="secondary-button" onClick={(e) => { e.stopPropagation(); onAdminPatch(tab === "templates" ? "document-templates" : tab, row.id, tab === "products" ? { price: Number(row.price || 0) + 1000 } : tab === "warehouses" ? { city: row.city } : tab === "uoms" ? { precision: row.precision } : tab === "users" ? { status: row.status || "active" } : { status: row.status || "active" }); }}>Update</button>
