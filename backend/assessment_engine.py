...
Ported/adapted from KN3 Discovery. Operates on a TEMPLATE dict (template-driven,
bilingual) instead of a static module so multiple templates can coexist.
"""
...
def get_all_question_ids(template):
    return [q["id"] for d in template.get("domains", []) for q in d.get("questions", [])]


def question_index(template):
    return {q["id"]: q for d in template.get("domains", []) for q in d.get("questions", [])}


def get_total_questions(template):
    return sum(len(d.get("questions", [])) for d in template.get("domains", []))
